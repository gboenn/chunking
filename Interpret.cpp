...
string Interpret::version_number = "1.3.4";
...
void Interpret::Dispatch(TextIO& coms, Modul& mdl, int from_cmd_line)
{
    ...
    if (ms == modulTable[kjump]) {
        if (args_obj[3].text[0] != '\0') {
            string s1 = args_obj[0].text;
            unsigned long i1 = stoul (args_obj[1].text);
    ...
    if (ms == modulTable[ksilence]) {
        if (args_obj[2].text[0] != '\0') {
            string s1 = args_obj[0].text;
            unsigned long i1 = stoul (args_obj[1].text);
            unsigned long i2 = stoul (args_obj[2].text);
    ...
    if (ms == modulTable[ktsnmr]) {
        if (args_obj[2].text[0] != '\0') {
            string s1 = args_obj[0].text;
            string s2 = args_obj[1].text;
            float f1 = atof (args_obj[2].text);
            mdl.Translate_Shorthand (s1, s2, f1);
        } else {
            cout << "Usage: chunking -m tsnmr <string snmr_file> <string pitch_file> <float pulses_per_bar>" << endl;
        }
    }
